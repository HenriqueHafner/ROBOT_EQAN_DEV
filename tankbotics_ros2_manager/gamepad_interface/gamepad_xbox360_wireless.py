...
def get_gamepad_interface_xbox360():
    ...
    if device_index_to_bind >= 0:
        ...
        gamepad_xbox_instance = gamepad(device_index_to_bind)
        print("Binding to device number: ", device_index_to_bind)
    else:
        print("No matching Xbox 360 controller found.")
    return gamepad_xbox_instance
...
class gamepad:
    ...
    def print_state(self, on_line_0 = False):
        if on_line_0:
            ...
            sys.stdout.write("### xbox360: " + self.get_stringfied_state()+"\n")
            sys.stdout.flush()
        else:
...
if __name__ == "__main__":
    xbox_instance = get_gamepad_interface_xbox360()
    if xbox_instance:
        ...
        while True:
            ...
            if delta_time >= 0.017:
                update_timestamp = time.monotonic()
                xbox_instance.update_state()
                xbox_instance.print_state()